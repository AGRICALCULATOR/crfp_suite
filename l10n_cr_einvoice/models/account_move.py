...
class AccountMove(models.Model):
    ...
    def _fp_lock_record_nowait(self):
        """Acquire a NOWAIT row lock to prevent concurrent FE processing."""
        self.ensure_one()
        self.env.cr.execute(
            f"SELECT id FROM {self._table} WHERE id = %s FOR UPDATE NOWAIT",
            [self.id],
        )

    def _fp_try_lock_record_nowait(self):
        """Best-effort row lock helper for cron/manual concurrent runs."""
        self.ensure_one()
        try:
            with self.env.cr.savepoint():
                self._fp_lock_record_nowait()
            return True
        except (
            ...
            _logger.info(
                "Documento FE %s bloqueado por otro proceso; se omite ejecución concurrente.",
                self.name,
            )
            return False
    ...
    def write(self, vals):
        ...
        if "invoice_line_ids" in vals and self and not self.env.context.get("fp_skip_other_charge_sync"):
            self._fp_sync_other_charge_accounting_lines()
        return res
    ...
    @api.onchange("fp_other_charge_line_ids")
    def _onchange_fp_other_charge_line_ids(self):
        self._fp_sync_other_charge_accounting_lines()
    ...
    def _onchange_fp_invoice_lines_recompute_other_charges(self):
        """Mantiene base/monto de otros cargos alineados con líneas en borrador.

        En formularios draft, el usuario puede editar líneas comerciales sin
        guardar. Forzamos recomputo in-memory para que ``untaxed_base_amount`` y
        ``amount`` reflejen la base real antes de generar/sincronizar líneas
        técnicas contables.
        """
        for move in self:
            if not move.fp_other_charge_line_ids:
                continue
            move.fp_other_charge_line_ids._compute_amount()
            move._fp_sync_other_charge_accounting_lines()

    def _fp_get_other_charge_receivable_line(self):
        self.ensure_one()
        receivable_lines = self.line_ids.filtered(
            lambda line: line.account_id.account_type == "asset_receivable" and not line.fp_is_other_charge_line
        )
        return receivable_lines[:1]

    def _fp_sync_other_charge_accounting_lines(self):
        for move in self:
            if not move.id or move.state != "draft" or move.move_type not in ("out_invoice", "out_refund"):
                continue

            # We remove previously generated accounting lines from ``line_ids``
            # (instead of ``invoice_line_ids``) because FE other-charge lines
            # are marked with ``exclude_from_invoice_tab`` and are therefore not
            # guaranteed to be present in the invoice tab relation.
            existing_lines = move.line_ids.filtered("fp_is_other_charge_line")
            commands = [(2, line.id, 0) for line in existing_lines]
            for charge in move.fp_other_charge_line_ids.filtered(lambda l: l.amount > 0.0 and l.account_id):
                commands.append(
                    (
                        0,
                        0,
                        {
                            "name": charge._fp_get_detail_label(),
                            "account_id": charge.account_id.id,
                            "quantity": 1.0,
                            "price_unit": charge.amount,
                            "tax_ids": [(5, 0, 0)],
                            "fp_is_other_charge_line": True,
                            "fp_other_charge_id": charge.id,
            ...
            if commands:
                move.with_context(
                    ...
                    fp_skip_other_charge_sync=True,
                ).write({"invoice_line_ids": commands})
                ...
                if hasattr(move, "_recompute_dynamic_lines"):
                    move._recompute_dynamic_lines(recompute_all_taxes=False)
                move._compute_amount()

    def _fp_validate_other_charges(self):
        self.ensure_one()
        lines = self.fp_other_charge_line_ids
        if not lines:
            return
        if self.fp_document_type not in ("FE", "TE", "NC"):
            raise ValidationError(_("Otros cargos solo aplica para FE, TE y NC."))
        for other_charge in lines:
            if not other_charge.detail:
                raise ValidationError(_("Cada otro cargo debe tener detalle."))
            if other_charge.amount <= 0.0:
                raise ValidationError(_("El monto de cada otro cargo debe ser mayor a cero."))
            if not other_charge.account_id:
                raise ValidationError(_("Cada otro cargo debe tener una cuenta contable."))
    ...
    def action_post(self):
        ...
        for move in electronic_moves:
            move._fp_validate_other_charges()
    ...
    def _get_invoice_report_filename(self, report=None):
        ...
        document_filename_base = self._fp_get_document_filename_base()
        if not document_filename_base:
            return filename

        if (filename or "") == document_filename_base:
            return filename
        return document_filename_base
    ...
    def _fp_add_hacienda_attachments_to_mail_action(self, action):
        ...
        for move in self.filtered("fp_is_electronic_invoice"):
            if move.fp_invoice_status == "accepted" and move.move_type in ("out_invoice", "out_refund"):
                try:
                    pdf_attachment = move._fp_ensure_invoice_pdf_attachment()
                except Exception:
                    _logger.exception("No fue posible preparar PDF para el asistente de correo de %s", move.name)
                    pdf_attachment = False
                if pdf_attachment:
                    attachments_by_id[pdf_attachment.id] = pdf_attachment
        ...
        existing_widget_values = context.get("default_mail_attachments_widget")
        # Avoid forcing ``default_mail_attachments_widget`` when the base
        # action did not provide it. In some Odoo versions, setting this key
        # too early bypasses report placeholders and the invoice PDF is not
        # generated/attached by the composer.
        if isinstance(existing_widget_values, list) and existing_widget_values:
            widget_by_id = {}
    ...
    def _fp_get_email_reference(self):
        ...
        self.ensure_one()
        if self.fp_is_electronic_invoice:
            return self.fp_consecutive_number or self._fp_extract_consecutive_from_clave(self.fp_external_id) or "n/a"
        return self.name or "n/a"

    def _fp_get_email_reference_for_body(self):
        """Return preferred reference identifier for outbound email body."""
        self.ensure_one()
        if self.fp_is_electronic_invoice:
            return self.fp_consecutive_number or self._fp_extract_consecutive_from_clave(self.fp_external_id) or ""
        return self.name or ""

    def _fp_get_invoice_report_action(self):
        self.ensure_one()
        report_xml_ids = (
        ...
        for report_xml_id in report_xml_ids:
            report = self.env.ref(report_xml_id, raise_if_not_found=False)
            if report:
                return report
        return False

    def _fp_ensure_invoice_pdf_attachment(self):
        self.ensure_one()
        if self.state != "posted":
            return False

        filename = f"{self._get_invoice_report_filename() or self.name or 'invoice'}.pdf"
        ...
        if attachment and attachment.with_context(bin_size=False).datas:
            return attachment

        report = self._fp_get_invoice_report_action()
        if not report:
            _logger.warning("No se encontró acción de reporte para generar PDF de factura %s", self.name)
            return False

        pdf_content, _content_type = report.sudo()._render_qweb_pdf(report.report_name, res_ids=self.ids)
        if not pdf_content:
            _logger.warning("Reporte de factura %s no devolvió contenido PDF", self.name)
            return False
        ...
        if attachment:
            ...
            return attachment
        return attachment_model.create(values)

    def _fp_get_hacienda_attachment_ids(self, ensure_pdf=False):
        self.ensure_one()
        attachment_ids = []
        if ensure_pdf and self.move_type in ("out_invoice", "out_refund"):
            pdf_attachment = self._fp_ensure_invoice_pdf_attachment()
            if pdf_attachment:
    ...
    def _fp_send_accepted_invoice_email(self):
        self.ensure_one()
        if not self._fp_try_lock_record_nowait():
            return False
        ...
        template = self.env.ref("l10n_cr_einvoice.email_template_edi_invoice_cr", raise_if_not_found=False)
        if not template:
            template = self.env.ref("account.email_template_edi_invoice", raise_if_not_found=False)
        if not template:
            return False

        document_label = self._fp_get_email_document_label()
        email_values = {
            "subject": _("%(company)s %(document)s (Ref %(reference)s)")
            % {
                "company": self.company_id.name or "",
                "document": document_label,
                "reference": self._fp_get_email_reference(),
            }
        }
        mail_id = template.send_mail(self.id, force_send=False, email_values=email_values)
        attachment_ids = self._fp_get_hacienda_attachment_ids(ensure_pdf=True)
        if mail_id:
    ...
    def _reverse_moves(self, default_values_list=None, cancel=False):
        ...
        if electronic_refunds:
            ...
            electronic_refunds._fp_cleanup_reversed_other_charge_lines()
        reversed_moves._fp_populate_reference_from_reversed_entry(force=True)
        return reversed_moves

    def _fp_cleanup_reversed_other_charge_lines(self):
        """Drop reversed invoice lines that belong to FE other charges.

        Safeguard for credit notes generated from historical invoices where
        technical markers may be inconsistent after reversal.
        """
        for move in self.filtered(lambda m: m.state == "draft" and m.move_type == "out_refund"):
            charge_lines = move.invoice_line_ids.filtered(
                lambda line: line.fp_is_other_charge_line or bool(line.fp_other_charge_id)
            )
            if charge_lines:
                move.with_context(
                    check_move_validity=False,
                    fp_skip_other_charge_sync=True,
                ).write({"invoice_line_ids": [(2, line.id, 0) for line in charge_lines]})
    ...
    fp_other_charge_line_ids = fields.One2many(
        ...
        "move_id",
        string="Otros cargos FE",
        copy=True,
    )
    fp_total_other_charges = fields.Monetary(
        string="Total otros cargos FE",
        currency_field="currency_id",
        compute="_compute_fp_total_other_charges",
        store=True,
    )
    fp_untaxed_base_amount = fields.Monetary(
        string="Base sin otros cargos FE",
        currency_field="currency_id",
        compute="_compute_fp_untaxed_base_amount",
        store=False,
        readonly=True,
    )
    ...
    def _compute_fp_total_other_charges(self):
        for move in self:
            move.fp_total_other_charges = sum(move.fp_other_charge_line_ids.mapped("amount"))
    ...
    @api.depends(
        ...
        "invoice_line_ids.display_type",
        "invoice_line_ids.exclude_from_invoice_tab",
        "invoice_line_ids.fp_is_other_charge_line",
        "invoice_line_ids.fp_other_charge_id",
        "invoice_line_ids.tax_line_id",
        "invoice_line_ids.account_id.account_type",
    )
    def _compute_fp_untaxed_base_amount(self):
        for move in self:
            move.fp_untaxed_base_amount = move._fp_get_untaxed_base_amount_for_other_charges()

    def _fp_get_untaxed_base_amount_for_other_charges(self):
        """Retorna la base comercial usada para calcular Otros Cargos FE.

        Se centraliza para reutilizar la misma lógica tanto en el compute del
        movimiento como en el cálculo por porcentaje de ``account.move.other.charge``.
        """
        self.ensure_one()
        # Nunca usar move.line_ids aquí: incluye líneas técnicas/contables.
        invoice_lines = self.invoice_line_ids.filtered(
            lambda line: not line.display_type
            and not line.tax_line_id
            and not line.fp_is_other_charge_line
        )
        tax_base_total = sum(invoice_lines.mapped("tax_base_amount"))
        if tax_base_total:
            return tax_base_total
        # Fallback consistente con el subtotal sin impuestos de la factura.
    ...
    def action_fp_consult_api_document(self):
        for move in self:
            if not move._fp_try_lock_record_nowait():
                continue
            ...
            if status == "aceptado":
                ...
                try:
                    move._fp_ensure_invoice_pdf_attachment()
                except Exception:
                    _logger.exception("No fue posible generar adjunto PDF para factura %s", move.name)
    ...
    def _fp_generate_invoice_xml(self, clave=None):
        ...
        if receptor_activity_code and self.fp_document_type != "TE":
        ...
        self._fp_validate_fe_receptor_required_data(receptor_partner, receptor_vat)
        receptor = ET.SubElement(root, "Receptor")
        ET.SubElement(receptor, "Nombre").text = receptor_name or ""
        receptor_has_identification = self._fp_append_identification_nodes(receptor, receptor_partner, receptor_vat, "receptor")
        self._fp_append_location_nodes(receptor, receptor_partner, "receptor", has_identification=receptor_has_identification)
        self._fp_append_contact_nodes(receptor, receptor_partner, "receptor", has_identification=receptor_has_identification)
        ...
        other_charge_lines = self._fp_get_xml_other_charges()
        total_other_charges = self._fp_append_other_charges_xml(root, other_charge_lines)
        ...
        include_no_sujeto_nodes = self.fp_document_type != "FEE"
        include_exonerado_nodes = self.fp_document_type != "FEE"
        ...
        if self.fp_document_type not in ("FEC", "FEE"):
        ...
        if total_other_charges:
            ET.SubElement(resumen, "TotalOtrosCargos").text = self._fp_format_decimal(total_other_charges)
        medio_pago = ET.SubElement(resumen, "MedioPago")
        ET.SubElement(medio_pago, "TipoMedioPago").text = self.fp_payment_method or "01"
        ET.SubElement(resumen, "TotalComprobante").text = self._fp_format_decimal(
            detalle_vals["total_comprobante"] + total_other_charges
        )
    ...
    def _fp_get_xml_other_charges(self):
        self.ensure_one()
        if self.fp_document_type not in ("FE", "TE", "NC"):
            return self.env["account.move.other.charge"]
        return self.fp_other_charge_line_ids.filtered(lambda l: l.amount > 0.0)

    def _fp_append_other_charges_xml(self, root, other_charges=None):
        self.ensure_one()
        other_charges = other_charges if other_charges is not None else self._fp_get_xml_other_charges()
        for other_charge in other_charges:
            node = ET.SubElement(root, "OtrosCargos")
            ET.SubElement(node, "TipoDocumentoOC").text = other_charge.document_type

            if other_charge.document_type == "99":
                tipo_documento_otros = (other_charge.document_type_other or "").strip()
                if len(tipo_documento_otros) < 5:
                    raise UserError(
                        _(
                            "El XML de Otros Cargos requiere 'Tipo documento OTROS' (mínimo 5 caracteres) cuando el tipo es 99."
                        )
                    )
                ET.SubElement(node, "TipoDocumentoOTROS").text = tipo_documento_otros

            third_party = other_charge.third_party_id
            third_party_vat = (third_party.vat or "").strip() if third_party else ""
            third_party_identification_payload = (
                self._fp_get_party_identification_payload(third_party, third_party_vat)
                if third_party
                else {}
            )
            if third_party_identification_payload:
                identification_node = ET.SubElement(node, "IdentificacionTercero")
                ET.SubElement(
                    identification_node,
                    "Tipo",
                ).text = third_party_identification_payload["tipoIdentificacion"]
                ET.SubElement(
                    identification_node,
                    "Numero",
                ).text = third_party_identification_payload["numeroIdentificacion"]

            third_party_name = (third_party.name or "").strip() if third_party else ""
            if third_party_name:
                ET.SubElement(node, "NombreTercero").text = third_party_name[:100]

            ET.SubElement(node, "Detalle").text = other_charge._fp_get_detail_label()
            if other_charge.calculation_type == "percentage":
                ET.SubElement(node, "PorcentajeOC").text = self._fp_format_decimal(other_charge.percentage or 0.0)
            ET.SubElement(node, "MontoCargo").text = self._fp_format_decimal(other_charge.amount)
        return sum(other_charges.mapped("amount"))
    ...
    def _fp_get_line_discount_nature(self, line):
        self.ensure_one()
        if line.fp_discount_nature:
            return line.fp_discount_nature.strip()[:80]
        if line.discount and line.discount > 0:
            return _("Descuento general")
        return _("Descuento aplicado")
    ...
    def _fp_build_detail_lines(self, lines_node):
        ...
        detail_lines = self.invoice_line_ids.filtered(
            lambda line: (not line.display_type or line.display_type == "product")
            and not line.fp_is_other_charge_line
        )
        if not detail_lines:
        ...
        for idx, line in enumerate(detail_lines, start=1):
            ...
            if product_tmpl and product_tmpl.fp_tariff_heading and self._fp_requires_tariff_heading():
                ET.SubElement(detail, "PartidaArancelaria").text = product_tmpl.fp_tariff_heading
            ...
            discount_amount = max(monto_total * discount_rate, 0.0)
            subtotal = monto_total - discount_amount
            ...
            if discount_amount > 0.0:
                descuento_node = ET.SubElement(detail, "Descuento")
                ET.SubElement(descuento_node, "MontoDescuento").text = self._fp_format_decimal(discount_amount)
                ET.SubElement(descuento_node, "CodigoDescuento").text = line.fp_discount_code or "07"
                ET.SubElement(descuento_node, "NaturalezaDescuento").text = self._fp_get_line_discount_nature(line)
            ...
            if has_tax and (total_impuesto_xml_linea > 0 or has_exoneration):
                if has_exoneration:
                    if is_service:
                        totals["total_serv_exonerado"] += monto_total
                    else:
                        totals["total_merc_exonerada"] += monto_total
                    totals["total_exonerado"] += monto_total
                else:
                    if is_service:
                        totals["total_serv_gravados"] += monto_total
                    else:
                        totals["total_mercancias_gravadas"] += monto_total
                    totals["total_gravado"] += monto_total
            elif has_tax and tax_rate_code in ("01", "05", "11"):
                if is_service:
                    totals["total_serv_no_sujeto"] += monto_total
                else:
                    totals["total_merc_no_sujeta"] += monto_total
                totals["total_no_sujeto"] += monto_total
            elif has_tax and tax_rate_code == "10":
                if is_service:
                    totals["total_serv_exentos"] += monto_total
                else:
                    totals["total_mercancias_exentas"] += monto_total
                totals["total_exento"] += monto_total
            else:
                # TEE usually carries lines without taxes configured in Odoo,
                # but Hacienda v4.4 expects those amounts in "No Sujeto"
                # totals instead of "Exento".
                if self.fp_document_type == "TEE":
                    if is_service:
                        totals["total_serv_no_sujeto"] += monto_total
                    else:
                        totals["total_merc_no_sujeta"] += monto_total
    ...
    def _fp_get_report_summary_totals(self):
        self.ensure_one()
        totals = self._fp_build_detail_lines(ET.Element("DetalleServicio"))
        other_total = sum(self._fp_get_xml_other_charges().mapped("amount"))
        totals["total_otros_cargos"] = other_total
        totals["total_comprobante"] += other_total
        return totals

    def _fp_get_report_tax_rate_breakdown(self):
        """Return report rows grouped by IVA rate for the corporate CR PDF."""
        self.ensure_one()
        detail_lines = self.invoice_line_ids.filtered(
            lambda line: (not line.display_type or line.display_type == "product")
            and not line.fp_is_other_charge_line
        ...
        for line in detail_lines:
            taxes = line.tax_ids
            tax = (
                taxes.filtered(lambda t: t.type_tax_use == expected_tax_use)
                or taxes.filtered(lambda t: t.type_tax_use == "none")
                or taxes
            )[:1]
            ...
            tax_rate = configured_tax_rate or odoo_tax_rate or code_tax_rate
            has_tax = bool(tax)
            has_exoneration = bool(self._fp_get_line_exoneration(line)) if has_tax else False

            subtotal = line.price_subtotal
            tax_amount = max(line.price_total - line.price_subtotal, 0.0)

            if has_tax and (tax_rate > 0.0 or has_exoneration):
                if has_exoneration:
                    group_key = ("label", "Exo")
                    group_label = "Exo"
                    sort_index = 1013.0
                else:
                    group_key = ("rate", tax_rate)
                    group_label = "{:.2f}%".format(tax_rate)
                    sort_index = tax_rate
            elif has_tax and tax_rate_code in ("01", "05", "11"):
                group_key = ("label", "NoS")
                group_label = "NoS"
            ...
            grouped_vals = breakdown_by_rate.setdefault(
                group_key,
                {"rate_label": group_label, "subtotal": 0.0, "tax": 0.0, "sort_index": sort_index},
            )
            grouped_vals["subtotal"] += subtotal
            grouped_vals["tax"] += tax_amount

        return [
            vals
            for vals in sorted(
                breakdown_by_rate.values(),
                key=lambda group: (group["sort_index"], group["rate_label"]),
            )
        ]
    ...
    def _fp_requires_tariff_heading(self):
        self.ensure_one()
        if self.fp_document_type in ("FEE", "TEE"):
            return True
        if self.fp_document_type == "NC" and self.reversed_entry_id:
            return self.reversed_entry_id.fp_document_type in ("FEE", "TEE")
        return False
    ...
    def _fp_format_decimal(self, value):
        decimal_value = Decimal(str(value or 0.0)).quantize(Decimal("0.00001"), rounding=ROUND_HALF_UP)
        return f"{decimal_value:.5f}"

    def _fp_receptor_allows_partial_data(self):
        self.ensure_one()
        ...
        if referenced_document_type == "TE":
            return True
        return self.fp_reference_document_type == "04"

    def _fp_is_nc_from_fee(self):
        self.ensure_one()
        if self.fp_document_type != "NC":
            return False
    ...
    def _fp_validate_fe_receptor_required_data(self, receptor_partner, receptor_vat):
        self.ensure_one()
        if self.fp_document_type != "FE":
            return

        missing_fields = []
        identification_type = (receptor_partner.fp_identification_type or "").strip()
        identification_number = self._fp_format_identification_number(receptor_vat, identification_type)
        if not identification_type:
            missing_fields.append(_("tipo de identificación"))
        if not identification_number:
            missing_fields.append(_("identificación"))

        province_source = receptor_partner.fp_province_id.code if receptor_partner.fp_province_id else ""
        province_source = province_source or (
            receptor_partner.state_id.code if receptor_partner.state_id and receptor_partner.state_id.code else receptor_partner.fp_province_code
        )
        canton_source = receptor_partner.fp_canton_id.code if receptor_partner.fp_canton_id else ""
        canton_source = canton_source or receptor_partner.fp_canton_code or receptor_partner.city
        district_source = receptor_partner.fp_district_id.code if receptor_partner.fp_district_id else ""
        district_source = district_source or receptor_partner.fp_district_code

        if not self._fp_pad_numeric_code_if_present(province_source, 1):
            missing_fields.append(_("provincia"))
        if not self._fp_pad_numeric_code_if_present(canton_source, 2):
            missing_fields.append(_("cantón"))
        if not self._fp_pad_numeric_code_if_present(district_source, 2):
            missing_fields.append(_("distrito"))
        if not (receptor_partner.street or "").strip():
            missing_fields.append(_("otras señas"))
        if not (receptor_partner.email or "").strip():
            missing_fields.append(_("correo electrónico"))

        if missing_fields:
            raise UserError(
                _(
                    "La Factura Electrónica (FE) requiere datos completos del receptor. "
                    "Faltan los siguientes campos: %(fields)s."
                )
    ...
    def _fp_append_identification_nodes(self, parent_node, partner, vat_source, party_role):
        ...
        if party_role == "receptor" and self._fp_receptor_allows_partial_data():
    ...
    def _fp_append_location_nodes(self, parent_node, partner, party_role, has_identification=False):
        if not partner:
            return

        identification_type = (partner.fp_identification_type or "").strip()

        if self.fp_document_type == "FEC" and party_role == "emisor" and identification_type == "05":
            foreign_other_signs = (partner.street or "").strip()[:160]
            if not foreign_other_signs:
                return
            ET.SubElement(parent_node, "OtrasSenasExtranjero").text = foreign_other_signs
            return
        ...
        is_export_receptor_without_location = (
            party_role == "receptor"
            and partner.country_id.code != "CR"
            and (self.fp_document_type == "FEE" or self._fp_is_nc_from_fee())
        )
        if is_export_receptor_without_location:
            return
    ...
    def _fp_append_contact_nodes(self, parent_node, partner, party_role=None, has_identification=False):
        if party_role == "receptor" and self._fp_receptor_allows_partial_data() and not has_identification:
            return
    ...
    def _fp_get_xml_filename_prefix(self, clave=None):
        self.ensure_one()
        return self._fp_get_document_filename_base(clave=clave)

    def _fp_get_document_filename_base(self, clave=None):
        self.ensure_one()
        consecutive = self.fp_consecutive_number
        if not consecutive:
            clave_value = clave or self.fp_external_id
            if clave_value:
        ...
        document_type = (self.fp_document_type or "DOC").strip().upper()
        return f"{document_type}-{consecutive}"
